...
void Renderer::initVulkan(std::span<const char*> instance_extensions)
{
  ...
  etna::initialize(etna::InitParams{
    .applicationName = "Diffuse baking demo",
...
void Renderer::initFrameDelivery(vk::UniqueSurfaceKHR a_surface, ResolutionProvider res_provider)
{
  ...
  diffuseBakingDemoRenderer = std::make_unique<DiffuseBakingDemoRenderer>();

  diffuseBakingDemoRenderer->allocateResources(resolution);
  diffuseBakingDemoRenderer->loadShaders();
  diffuseBakingDemoRenderer->setupPipelines(window->getCurrentFormat());
...
{
  diffuseBakingDemoRenderer->debugInput(kb);
...
{
  diffuseBakingDemoRenderer->update(packet);
}
...
void Renderer::drawFrame()
{
...

  {
    ...
    ImGui::NewFrame();
    diffuseBakingDemoRenderer->drawGui();
    ImGui::Render();
  }
  ...
  if (nextSwapchainImage)
  {
    ...
    ETNA_CHECK_VK_RESULT(currentCmdBuf.begin(vk::CommandBufferBeginInfo{}));
    {
      ...
      diffuseBakingDemoRenderer->renderWorld(currentCmdBuf, image, view);

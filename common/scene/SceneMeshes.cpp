  ...
  etna::OneShotCmdMgr& one_shot_cmd_mgr)
{
  ...
  for (std::size_t i = 0; i < model.nodes.size(); ++i)
  {
    if (model.nodes[i].mesh >= 0)
    {
      meshInstances.push_back(MeshInstance{
        ...
        .mesh = static_cast<std::uint32_t>(model.nodes[i].mesh),
      });
    }
  }
  ...
  // allocator on the memcpy hotpath
  {
    ...
    indices.reserve(indexBytes / sizeof(std::uint32_t));
  }
...

  {
    ...
    renderElements.reserve(totalPrimitives);
  }
  ...
  for (const auto& mesh : model.meshes)
  {
    ...
    for (const auto& prim : mesh.primitives)
    {
      ...
      renderElements.push_back(RenderElement{
        .vertexOffset = static_cast<std::uint32_t>(vertices.size()),
        .indexOffset = static_cast<std::uint32_t>(indices.size()),
  ...
  transfer_helper.uploadBuffer<Vertex>(one_shot_cmd_mgr, vertexBuffer, 0, vertices);
  transfer_helper.uploadBuffer<std::uint32_t>(one_shot_cmd_mgr, indexBuffer, 0, indices);
}

etna::VertexByteStreamFormatDescription SceneMeshes::getVertexFormatDescription() const
{
